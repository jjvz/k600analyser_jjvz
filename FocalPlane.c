...
*/
     ...
     res[1]=drift_min_esti- dd[wireID_min];
     //res[1] is difference in position calculated at wire of min drifttime, and measured at that wire
...
//lineshape correction
{
  ...
  *Xcorr = 0;
  ...
  for(int i=0;i<NXY1Corr;i++)
    {
      if(i==0)result = result + XY1Corr[i];
      if(i>0)result += XY1Corr[i] * pow(Y,i);	// result = result + XY1Corr[0] + XY1Corr[1]*Y;  // where XY1Corr[0] = 0.833 and XY1Corr[1] = 0.098
    }
...
//  result = 31.3029 - 0.0299994*Xcorr - 1.54103e-6*pow(Xcorr,2.);
  result = 30.69589 - 0.0314497*Xcorr - 6.69387e-7*pow(Xcorr,2.);
  return result;
}
