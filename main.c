...
/* ---------------------------------------------------------------------------------------*/
void read_tof(int *run_tof_off, char* fileName) 
{
   FILE *file = fopen(fileName, "r");
   //assert(file != NULL);
   short nruns = 7000; //akin to number of tdc channels, but not physical chan. 
   char string_var[200] = "";
   int int_var = 0;
   int tofoffsets[7000];

   for(int i = 0; i < nruns; i ++) {
      if (feof(file)) break; //just in case
  
      fscanf(file,"%s", string_var);
      int_var = atoi(string_var);
      //printf("run #: %i \t int_Var : %i  \n",i+1, int_var);
      tofoffsets[i] = int_var;
      //printf("int # %i \n",tofoffsets[i]);
   }
   GetODBRunInfo();
   *run_tof_off = tofoffsets[runinfo2.run_number-1];
   //printf("----------------TOF offset (inside) for run#: %i is %i  -----------\n",runinfo2.run_number, *run_tof_off );                
   fclose(file);   
}
/* ---------------------------------------------------------------------------------------*/
...
// ************** adjust TOF loci per run - offset values given in file tof_offset.dat ************************ 
   read_tof(&tofOffset,"tof_offset.dat");
   printf("== TOF offset: %i ==\n\n",tofOffset);
//*************************************************************************************************************
   ...
   // in terms of kinematics. So now I use the O calibration but offsets the X1pos to compensate...
		...
		case 6028: x1offset=3.95; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
		case 6029: x1offset=3.98; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
		case 6036: x1offset=3.43; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
		case 6037: x1offset=3.5; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
		case 6038: x1offset=3.43; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
		case 6039: x1offset=4.4; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
		case 6040: x1offset=4.45; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
		case 6042: x1offset=3.3; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
		case 6043: x1offset=3.43; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
		case 6044: x1offset=3.27; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
		case 6045: x1offset=3.6; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
		case 6048: x1offset=3.55; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
		case 6049: x1offset=2.87; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
		case 6050: x1offset=2.33; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
		case 6052: x1offset=0.83; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
		case 6054: x1offset=0.; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
		case 6055: x1offset=0.; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
		case 6056: x1offset=-0.7; printf("run %d: x1offset = %f \n",runinfo2.run_number,x1offset); break;
   ...
   for(int i = 0; i < ntdc; i++) {
	...
	hTDC2DModule[tdcmodule]->Fill(ref_time,channel); 		
      ...
      switch(channel){
		...
		// "+tofOffset" added above and below by JJvZ for TOF offset, Dec 2014
		case (TOF_TDC_CHAN+1*128): if(t_toftdc2==0) toftdc2=ref_time+tofOffset; t_toftdc2=toftdc2; break;
		case (TOF_TDC_CHAN+2*128): if(t_toftdc3==0) toftdc3=ref_time+tofOffset; t_toftdc3=toftdc3; break;
		case (TOF_TDC_CHAN+3*128): if(t_toftdc4==0) toftdc4=ref_time+tofOffset; t_toftdc4=toftdc4; break;
		case (TOF_TDC_CHAN+4*128): if(t_toftdc5==0) toftdc5=ref_time+tofOffset; t_toftdc5=toftdc5; break;
		case (TOF_TDC_CHAN+5*128): if(t_toftdc6==0) toftdc6=ref_time+tofOffset; t_toftdc6=toftdc6; break;
		case (TOF_TDC_CHAN+6*128): if(t_toftdc7==0) toftdc7=ref_time+tofOffset; t_toftdc7=toftdc7; break;
		#ifdef _POLARIZATION  		
