{
   Int_t nrpeaks=3;
   Double_t par[9]; 
//   Float_t min1=500, max1=2000, sig=70., delt_search=200., delt_plot=300.; //for 120 MeV, 35 deg
   Float_t min1=100, max1=400, sig=9., delt_search=10., delt_plot=30.; //for 120 MeV, 35 deg
   Float_t cent[3]={182.,275.,300.}; 
//   Float_t cent[3]={662.,1173.,1332.}; 
   Float_t N0[3]={30000,10000,10000};
...
// func->SetParLimits(2,cent[0]-delt_search,cent[0]+delt_search);
...
// total->SetParameters(N0[0],cent[0],sig,N0[1],cent[1],sig,N0[2],cent[2],sig);
   ...
   total->SetParLimits(1,cent[0]-delt_search,cent[0]+delt_search);
   total->SetParLimits(4,cent[1]-delt_search,cent[1]+delt_search);
   total->SetParLimits(7,cent[2]-delt_search,cent[2]+delt_search);
...
*/
   ...
   for(m=0;m<3*nrpeaks;++m) { cout<<"par["<<m<<"]="<<par[m]<<";  "; }
   cout<<endl;
